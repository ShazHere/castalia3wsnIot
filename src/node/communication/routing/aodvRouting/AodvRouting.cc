...
void AodvRouting::processDataPackage(AodvDataPacket* pkt){
	...
	if(string(pkt->getDestinationAodv()).compare(SELF_NETWORK_ADDRESS)==0){ // it is a final dest, then pass to app layer
		toApplicationLayer(pkt->decapsulate());
		return;
        // if it is not a final dest but has a route to final, then send to next hop from routing table
		...
		return;
       // if it does not have valid route to a final, then if no ongoing request is valid, send RREQ to discover new route to final dest
		...
		//buffer the packet for further opretaion 
